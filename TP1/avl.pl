...
left_rotate(avl(G,R,D,_), A_Apres) :-
	...
	%---------------------------------
	% On suppose que larbre avant insertion est equilibr� (difference de hauteur
...
insert(Elem, AVL, NEW_AVL) :-
	...
	(Elem = Racine ->
			% lelement est d�j� present, pas dinsertion possible
		fail
	;
		(Elem @< Racine ->
...
suppress(Elem, AVL, NEW_AVL) :-
	...
	%-------------------------------------------------------
	% Si lavl est vide, le pr�dicat �choue
...
suppress_max(Max, AVL, NEW_AVL) :-
	...
	% - soit apres suppression dun �l�ment dans le sous-arbre gauche
	%----------------------------------------------------------------
...
left_balance(Avl, New_Avl) :-
	...
	% - soit apres suppression dun �l�ment dans le sous-arbre droite
	%----------------------------------------------------------------
