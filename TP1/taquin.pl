...
Doit contenir au moins 4 predicats qui seront utilises par A*
   ...
   etat_initial(I)                                         % definit letat initial

   etat_final(F)                                           % definit letat final  

   rule(Rule_Name, Rule_Cost, Before_State, After_State)   % règles applicables

   heuristique(Current_State, Hval)				           % calcul de lheuristique 
...
% On peut également comparer les pieces qui se trouvent aux memes coordonnees dans U et dans H et voir s il sagit de la
% meme piece.
